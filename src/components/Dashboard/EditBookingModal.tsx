...
interface EditBookingModalProps {
  booking: any;
  onClose: () => void;
  onUpdateSuccess: () => void; // will refresh table
}

// Generate start/end time slots same as ReservationModal
...
const generateEndSlots = () => {
  ...
  for (let h = 0; h < 24; h++) {
    [15, 30, 45, 60].forEach((m) => {
      ...
      } else ts.push(`${String(h).padStart(2, "0")}:${String(m).padStart(2, "0")}`);
    });
  }
  return ts;
};
...
const EditBookingModal: React.FC<EditBookingModalProps> = ({ booking, onClose, onUpdateSuccess }) => {
  ...
  const [date, setDate] = useState(booking.date_reserved || new Date().toISOString().split("T")[0]);
  ...
  const fetchReservationsForDay = async () => {
    try {
      ...
      const mappedReservations = (data.reservations || [])
        ...
        .map((r: any) => ({ ...r, start_time: r.reservation_start, end_time: r.reservation_end }));
  ...
  const availableStartTimes = useMemo(() => ALL_START_SLOTS.filter((t) => isSlotAvailable(t)), [reservations, date]);
 ...
 const handleUpdate = async () => {
  try {
    const res = await fetch(`http://localhost:5000/api/room_bookings/${booking.id}`, {
      ...
      body: JSON.stringify({ date, startTime, endTime, notes, roomId: booking.room_id }), // ✅ include roomId if needed
    });

    if (res.ok) {
      alert("Booking updated successfully!");
      onUpdateSuccess(); // ✅ triggers parent refresh
      onClose();         // close modal
    } else {
  ...
  return (
    <div className="modal-overlay">
      <div className="modal-content">
        ...
        <p>
          <strong>Building:</strong> {booking.building_name} · <strong>Floor:</strong> {booking.floor_number} · <strong>Room:</strong> {booking.room_name}
        </p>
          ...
          <>
            ...
            {reservations.length > 0 && (
              <div className="mb-2">
                ...
                <ul>{reservations.map((r) => (<li key={r.id}>{r.start_time} - {r.end_time} ({r.reserved_by})</li>))}</ul>
              </div>
            )}
        ...
        {showConfirm && (
          <div className="mt-4 p-4 border rounded bg-gray-50">
            ...
            <div>
              <div><strong>Building:</strong> {booking.building_name}</div>
              <div><strong>Floor:</strong> {booking.floor_number}</div>
              <div><strong>Room:</strong> {booking.room_name}</div>
              <div><strong>Date:</strong> {date}</div>
              <div><strong>Time:</strong> {startTime} - {endTime}</div>
              <div><strong>Notes:</strong> {notes}</div>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: "10px" }}>
              <button className="px-3 py-2 border rounded" onClick={() => { setShowConfirm(false); setShowForm(true); }}>Cancel</button>
              <button className="px-3 py-2 bg-blue-600 text-white rounded" onClick={handleUpdate}>Update</button>
            </div>
          </div>
        )}
      </div>
