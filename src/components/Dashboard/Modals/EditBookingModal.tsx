...
export default function EditRoomModal({ room, onClose, onSuccess }: Props) {
  ...
  const save = async () => {
    ...
    try {
      ...
      // ✅ Fetch updated room from server to ensure RoomsTab refresh
      const updatedRes = await fetch(`/api/rooms/${room.id}`);
      if (!updatedRes.ok) throw new Error("Failed to fetch updated room");
      const updatedRoom: Room = await updatedRes.json();

      onSuccess(updatedRoom); // Updates parent roomList instantly
      onClose();
    } catch (err) {
      console.error(err);
